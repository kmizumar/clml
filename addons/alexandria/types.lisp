       ...
       (let ((subtype-names (list))
		...
		(make-docstring (range-beg range-end range-type)
		  (let ((inf (ecase range-type (:negative "-inf") (:positive "+inf"))))
           ...
           (let* ((negative-name     (make-subtype-name #-allegro "NEGATIVE-~A"     #+allegro (if (eq excl::*current-case-mode* :case-sensitive-lower) "negative-~a"     "NEGATIVE-~A")))
                  (non-positive-name (make-subtype-name #-allegro "NON-POSITIVE-~A" #+allegro (if (eq excl::*current-case-mode* :case-sensitive-lower) "non-positive-~a" "NON-POSITIVE-~A")))
                  (non-negative-name (make-subtype-name #-allegro "NON-NEGATIVE-~A" #+allegro (if (eq excl::*current-case-mode* :case-sensitive-lower) "non-negative-~a" "NON-NEGATIVE-~A")))
                  (positive-name     (make-subtype-name #-allegro "POSITIVE-~A"     #+allegro (if (eq excl::*current-case-mode* :case-sensitive-lower) "positive-~a"     "POSITIVE-~A")))
